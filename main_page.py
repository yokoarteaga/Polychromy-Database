...
('Pentelic', 'Paros', None), # MARBLES
 ('Egg white', 'Punic wax', 'Not applied', None), # BINDERS
 ('Cinnabar',
 ...
 'yellow ochre with gypsum (Zecchi)', 'Not applied', None), # PIGMENTS # ADDED 'NONE'! 
 ('Calcium carbonate',
 'Egyptian blue with calcite',
 'Lead white',
 'Mixed with calcite',
 'Not applied',None), # GROUNDS
 ("1", "2", "3","many","Not applied", None) # NLAYERS
 ) 
hide_menu = """
<style>
#MainMenu {
...
def show_shaders(name): 
   ...
   # file_path_png = os.path.join(folder_path, file_name)
    ...
    cols[0].image(image=file_path_gif, caption="AxF shader")    # gif for blender 
    ...
    # --- modifying for mode "shaders"
        ...
        if st.session_state.selected_names:
            ...
            with downloadBlender_button_area:
                ...
                st.download_button( label='Download Blender', data=download_file_from_zenodo(name, key_ID=blend_ID, ext="blend"), file_name=f'{name}.zip', mime="application/octet-stream")
